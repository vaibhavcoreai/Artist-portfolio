        ...
        <div className="w-full max-w-sm bg-deep-charcoal border border-white/10 p-8">
          ...
          <form onSubmit={handleLogin} className="flex flex-col space-y-6">
            <div className="flex flex-col space-y-2">
              <label className="text-xs uppercase tracking-widest text-ghost-white/60">Password</label>
              <input 
                type="password"
                value={password}
            ...
            <button 
              type="submit"
              className="w-full py-4 bg-aged-gold text-near-black font-sans uppercase tracking-[0.15em] text-xs font-medium hover:bg-warm-ivory transition-colors cursor-pointer"
            >
              Enter Studio
            </button>
          </form>
    ...
    <div className="min-h-screen bg-near-black flex flex-col md:flex-row font-sans cursor-auto">
      ...
      <aside className="w-full md:w-64 bg-deep-charcoal border-r border-white/5 flex flex-col p-8 shrink-0">
        <div className="mb-12">
          ...
          <p className="font-sans text-[10px] tracking-[0.2em] uppercase text-aged-gold">Admin Studio</p>
        </div>
        ...
        <nav className="flex flex-col space-y-4 flex-1">
          {['Artworks', 'Inquiries', 'Settings'].map(tab => (
            <NavLink
              ...
              className={({ isActive }) => `text-sm tracking-widest uppercase transition-colors py-2 ${isActive ? 'text-aged-gold' : 'text-ghost-white/50 hover:text-ghost-white'}`}
            >
              {tab}
            </NavLink>
          ))}
        ...
        <button 
          ...
          className="text-xs uppercase tracking-widest text-ghost-white/30 hover:text-white text-left mt-auto pt-8 border-t border-white/5"
        >
          Logout
        </button>
      </aside>
